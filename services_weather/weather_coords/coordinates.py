...
async def handle_user_location(message: Message):
    ...
    # 1. Отправляет сообщение о начале процесса.
    ...
    try:
        ...
        # Вызывает погоду через API - JSON по координатам.
        weather_dict = get_weather_by_coords(lat, lon)

        # ПРОВЕРКА: Если API вернуло None (ошибка в get_weather_by_coords).
        if not weather_dict:
        ...
        # Пытаемся удалить сообщение об ожидании, если оно еще есть.
        try:
            await wait_msg.delete()
        except: # noqa
            pass
