...
def get_full_finance_report():
    ...
    # 3. Курс usd для крипты.
    usd_value = currency_manager.get_usd_value()

    # 4. Создание запроса на нефть.
    manager = OilDataManager()
    oil_table = manager.fetch_oil_price()

    # --- СБОРКА ТЕКСТОВЫХ БЛОКОВ ---
    # Раздача данных функциям.
    # 4. Валютный отчет. Передача всего объекта в отчет по валютам.
    currency = get_currency_report(currency_manager)

    # 5. Крипто отчет. Передача всего объекта и числа (курс usd) в отчет по крипте.
    crypto = get_crypto_report(crypto_manager, usd_value)

    # 6. Нефтяной отчет. Функция сама создаст менеджера внутри и сходит на MOEX.
    oil = OilFormatter.get_oil_report(oil_table)

    # 7. Металлы. Форматтер сам разберется с ЦБ РФ внутри.
    metals = MetalFormatter.get_metals_report()
